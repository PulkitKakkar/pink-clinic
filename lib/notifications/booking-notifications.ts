...
function notificationLocation(booking: Booking) {
  const branchName =
    branchNotificationNames[booking.branchId] ||
    branches.find((item) => item.id === booking.branchId)?.name;
...
function notificationCopy(booking: Booking, type: BookingNotificationType) {
  ...
  if (type === "booking-cancelled")
    return {
      subject: "Your Pink Beauty booking has been cancelled",
      message: `Hi ${booking.customerName}, your booking for ${booking.treatmentName} at ${notificationLocation(booking)} on ${appointment} has been cancelled. Please contact us if you need help. ${footer}`,
    };
...
function buildNotification(
  ...
  return {
    ...
    booking,
    channels,
    ...notificationCopy(booking, type),
  };
}

async function readLocalDeliveries(): Promise<LocalDelivery[]> {
  try {
    return JSON.parse(await readFile(localFile, "utf8")) as LocalDelivery[];
  } catch {
    return [];
  }
}
...
async function claimDelivery(
  ...
  if (deliveries.some((item) => item.key === key && item.status === "sent"))
...
async function finishDelivery(
  ...
  const deliveries = await readLocalDeliveries();
  const next = [
    ...deliveries.filter((item) => item.key !== claim.id),
    { key: claim.id, status, error, updatedAt: new Date().toISOString() },
  ];
...
export async function sendBookingNotification(
  ...
  if (!notification.channels.length)
    return {
      sent: false,
      reason: "Customer has no email address or phone number.",
    };
  const claim = await claimDelivery(booking, type);
  if (!claim.claimed)
    return { sent: false, reason: "Notification already sent." };
  try {
...
export async function sendDueBookingReminders(
  ...
  for (const booking of bookings) {
    ...
    const hoursUntil =
      (new Date(booking.startsAt).getTime() - now.getTime()) / 3_600_000;
    const type =
      hoursUntil > 24 && hoursUntil <= 48 ? "reminder-48-hours" : undefined;
    if (!type) continue;
    try {
      results.push({
