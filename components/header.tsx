...
const links = [["Treatments", "/services"], ["Academy", "/courses"], ["Results", "/#results"], ["About", "/#why"], ["Locations", "/locations/west-street-reading"]];
  ...
  return (
    <header className="absolute inset-x-0 top-0 z-50 border-b border-white/15 text-white">
      <div className="container-site flex h-20 items-center justify-between">
        ...
        <div className="hidden lg:block"><BookingLink className="button-light" label="Book now" intent={{ source: "header" }} /></div>
      ...
      {open && <nav className="mx-4 rounded-3xl bg-white p-5 text-ink shadow-luxe lg:hidden">{links.map(([label, href]) => <Link onClick={() => setOpen(false)} key={label} href={href} className="block border-b border-black/5 py-4 font-semibold">{label}</Link>)}<BookingLink className="button-primary mt-5 w-full" intent={{ source: "mobile-menu" }} /></nav>}
    </header>
  );
}
