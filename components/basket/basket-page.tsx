...
export function BasketPage() {
  ...
  return <main>
    ...
    <section className="section-shell bg-cream"><div className="container-site">
      {!items.length ? <div className="rounded-[2rem] bg-white p-10 text-center shadow-soft sm:p-16"><ShoppingBag className="mx-auto text-pink" /><h2 className="mt-5 font-display text-4xl">Your basket is empty.</h2><p className="mx-auto mt-3 max-w-md text-sm leading-6 text-black/45">Browse products and services, choose an option and add it here.</p><Link href="/treatments/select-branch" className="button-primary mt-6">Choose a branch</Link></div> :
      <div className="grid gap-6 lg:grid-cols-[1fr_360px] lg:items-start">
        ...
        <aside className="rounded-[2rem] bg-[#210013] p-6 text-white shadow-soft sm:p-8 lg:sticky lg:top-6"><p className="text-[10px] font-bold uppercase tracking-[.25em] text-pink-light">Basket summary</p><div className="mt-6 flex items-center justify-between border-y border-white/10 py-5"><span className="text-sm text-white/55">Total</span><span className="text-3xl font-bold">£{total.toFixed(2)}</span></div><Link href={`/checkout/${branch?.slug}/basket`} className="button-primary mt-6 w-full">Continue to checkout</Link><p className="mt-4 text-center text-[10px] leading-5 text-white/40">Payment is routed securely to {branch?.name}.</p></aside>
      </div>}
    </div></section>
  </main>;
}
