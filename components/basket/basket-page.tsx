  ...
  return <main>
    ...
    <section className="section-shell bg-cream"><div className="container-site">
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_360px] lg:items-start">
        <div className="grid gap-3">{items.map((item) => <article key={item.id} className="grid grid-cols-[82px_1fr] gap-4 rounded-2xl bg-white p-3 shadow-soft sm:grid-cols-[110px_1fr] sm:p-4"><div className="relative min-h-24 overflow-hidden rounded-xl bg-pink-light">{item.image && <Image src={item.image} alt="" fill className="object-cover" sizes="110px" />}</div><div className="flex min-w-0 flex-col justify-between gap-3 sm:flex-row sm:items-center"><div><h2 className="font-display text-2xl leading-none">{item.title}</h2><p className="mt-2 text-xs text-black/45">{item.variantName}</p><p className="mt-2 text-sm font-bold text-pink">£{item.unitPrice.toFixed(2)}</p></div><div className="flex items-center justify-between gap-3 sm:justify-end"><div className="flex items-center rounded-full border border-black/10"><button type="button" onClick={() => updateQuantity(item.id, item.quantity - 1)} aria-label={`Reduce ${item.title} quantity`} className="grid h-11 w-11 place-items-center"><Minus size={13} /></button><span className="min-w-7 text-center text-xs font-bold">{item.quantity}</span><button type="button" onClick={() => updateQuantity(item.id, item.quantity + 1)} aria-label={`Increase ${item.title} quantity`} className="grid h-11 w-11 place-items-center"><Plus size={13} /></button></div><button type="button" onClick={() => removeItem(item.id)} aria-label={`Remove ${item.title}`} className="grid h-11 w-11 place-items-center rounded-full text-black/60 transition hover:bg-pink-light hover:text-pink"><Trash2 size={15} /></button></div></div></article>)}</div>
