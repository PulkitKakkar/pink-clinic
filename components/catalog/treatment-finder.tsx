...
const downtimeOptions = ["Minimal downtime", "A few days is fine", "Open to practitioner advice"];
const priorityOptions = ["A subtle refresh", "A targeted concern", "A longer-term plan"];
...
export function TreatmentFinder({ items }: { items: CatalogItem[] }) {
  ...
  return <div className="grid gap-8 lg:grid-cols-[.75fr_1.25fr] lg:gap-12">
    <div className="rounded-[2rem] bg-white p-5 shadow-soft sm:p-8">
      <FinderQuestion number="1" title="What would you like help with?" value={concern} options={treatmentConcerns.map((entry) => ({ value: entry.slug, label: entry.shortName }))} onChange={setConcern} />
      <FinderQuestion number="2" title="How do you feel about downtime?" value={downtime} options={downtimeOptions.map((label) => ({ value: label, label }))} onChange={setDowntime} />
      <FinderQuestion number="3" title="What matters most?" value={priority} options={priorityOptions.map((label) => ({ value: label, label }))} onChange={setPriority} />
...
function FinderQuestion({ number, title, value, options, onChange }: { number: string; title: string; value: string; options: { value: string; label: string }[]; onChange: (value: string) => void }) {
  return <fieldset className="border-b border-black/5 py-6 first:pt-0 last:border-0"><legend className="flex gap-3 text-sm font-bold"><span className="text-pink">{number}.</span>{title}</legend><div className="mt-4 flex flex-wrap gap-2">{options.map((option) => <button key={option.value} type="button" aria-pressed={value === option.value} onClick={() => onChange(option.value)} className={`min-h-11 rounded-full px-4 text-xs font-bold transition ${value === option.value ? "bg-pink text-white" : "bg-cream text-black/60 hover:text-pink"}`}>{option.label}</button>)}</div></fieldset>;
}
