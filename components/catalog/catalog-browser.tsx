...
export function CatalogBrowser({ items, branchId, branchSlug }: { items: CatalogItem[]; branchId: string; branchSlug: string }) {
  ...
  return <div>
    <div className="grid gap-4 rounded-2xl bg-pink-light/45 p-4 sm:p-5">
      ...
      <div className="flex flex-wrap gap-2">{Object.entries(typeLabels).map(([value, label]) => <button key={value} type="button" aria-pressed={type === value} onClick={() => { setType(value); setVisibleCount(12); }} className={`min-h-11 rounded-full px-4 py-2 text-[10px] font-bold uppercase tracking-[.12em] transition ${type === value ? "bg-pink text-white" : "bg-white text-black/55 hover:text-pink"}`}>{label}</button>)}</div>
    </div>
    {!query && type === "all" && <section className="mt-8"><div className="flex items-end justify-between gap-4"><div><p className="eyebrow">Our services</p><h2 className="font-display text-4xl">Browse collections</h2></div>{collection !== "all" && <button type="button" onClick={() => { setCollection("all"); setVisibleCount(12); }} className="text-xs font-bold text-pink">Clear</button>}</div><div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">{collectionCards.map(({ name, item }, index) => <button key={name} type="button" aria-pressed={collection === name} onClick={() => { setCollection(name); setVisibleCount(12); }} className="group relative min-h-32 overflow-hidden rounded-2xl bg-[#210013] p-4 text-left text-white">{item?.images[0] && <Image src={item.images[0]} alt="" fill priority={index === 0} className="object-cover opacity-45 transition group-hover:scale-105" sizes="(min-width: 640px) 25vw, 50vw" />}<span className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent" /><span className="relative flex h-full items-end font-display text-xl leading-none">{name}</span></button>)}</div><details className="mt-4 rounded-2xl border border-black/5 bg-white p-4"><summary className="cursor-pointer text-xs font-bold text-pink">View all collections</summary><div className="mt-4 flex flex-wrap gap-2"><button type="button" aria-pressed={collection === "all"} onClick={() => { setCollection("all"); setVisibleCount(12); }} className={`min-h-11 rounded-full px-4 text-[10px] font-bold ${collection === "all" ? "bg-pink text-white" : "bg-cream"}`}>All collections</button>{collections.map((name) => <button key={name} type="button" aria-pressed={collection === name} onClick={() => { setCollection(name); setVisibleCount(12); }} className={`min-h-11 rounded-full px-4 text-[10px] font-bold ${collection === name ? "bg-pink text-white" : "bg-cream"}`}>{name}</button>)}</div></details></section>}
