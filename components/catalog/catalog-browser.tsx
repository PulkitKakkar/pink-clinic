  ...
  return <div>
    ...
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
      {filtered.map((item) => {
        ...
        return <article key={item.handle} className="group overflow-hidden rounded-2xl border border-black/5 bg-white shadow-soft">
          ...
          <div className="p-5">
            ...
            {variants.length ? <div className="mt-4 grid gap-2"><button type="button" onClick={() => addCatalogItem(item, variants)} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-full bg-pink px-4 text-xs font-bold text-white transition hover:bg-pink-dark">{justAdded ? <><Check size={14} /> Added</> : <><ShoppingBag size={14} /> Add to basket</>}</button><Link href={`/checkout/${branchSlug}/catalog/${encodeURIComponent(item.handle)}`} className="text-center text-[10px] font-bold text-pink underline underline-offset-4">Buy now</Link></div> : <Link href={`/contact?branchId=${branchId}&catalogItem=${encodeURIComponent(item.handle)}`} className="mt-5 inline-flex text-xs font-bold text-pink underline underline-offset-4">Enquire</Link>}
          </div>
        </article>;
      })}
    </div>
