...
function getPriceDisplay(item: CatalogItem, alwaysFrom = false) {
  ...
  const cheapest = pricedVariants.reduce((lowest, variant) =>
    variant.price < lowest.price ? variant : lowest,
  );
  const hasRange = pricedVariants.some(
    (variant) => variant.price !== cheapest.price,
  );
  const original =
    cheapest.compareAtPrice && cheapest.compareAtPrice > cheapest.price
      ...
      : undefined;
  return {
    current: `${alwaysFrom || hasRange ? "From " : ""}£${cheapest.price.toFixed(2)}`,
    original,
  };
}
...
export function CatalogBrowser({
  ...
  const collections = useMemo(
    () =>
      [...new Set(items.flatMap((item) => item.tags))]
        .filter(Boolean)
        .sort((a, b) =>
          a === "Offers" ? -1 : b === "Offers" ? 1 : a.localeCompare(b),
        ),
    [items],
  );
  ...
  const collectionCards = useMemo(
    () =>
      collections.slice(0, 8).map((name) => ({
        name,
        item: items.find((item) => item.tags.includes(name) && item.images[0]),
      })),
    [collections, items],
  );
    ...
    () =>
      treatmentConcerns
        .map((entry) => ({
          ...entry,
          item: items.find(
            (item) => matchesConcern(item, entry) && item.images[0],
          ),
        }))
        .filter((entry) => entry.item),
    [items],
  );
  const serviceAreaCards = useMemo(
    () =>
      beautyServiceAreas
        .map((entry) => ({
          ...entry,
          item: items.find(
            (item) => matchesBeautyServiceArea(item, entry) && item.images[0],
          ),
        }))
        .filter((entry) => entry.item),
    [items],
  ...
  useEffect(() => {
    ...
    if (serviceArea === "all") url.searchParams.delete("serviceArea");
  ...
  return (
    <div>
        ...
        {!hideTypeFilters && (
          <div className="flex flex-wrap gap-2">
            {Object.entries(typeLabels)
              .filter(
                ([value]) =>
                  value === "all" || items.some((item) => item.kind === value),
              )
              .map(([value, label]) => (
                <button
                  key={value}
                  ...
                  className={`min-h-11 rounded-full px-4 py-2 text-[10px] font-bold uppercase tracking-[.12em] transition ${type === value ? "bg-pink text-white" : "bg-white text-black/55 hover:text-pink"}`}
                >
                  {label}
                </button>
              ))}
        ...
        <section className="mt-8">
          <div className="flex items-end justify-between gap-4">
            <div>
              ...
              <h2 className="font-display text-4xl">Treatments by concern</h2>
            </div>
            {concern !== "all" && (
              <button
                type="button"
          ...
          <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-3">
            {concernCards.map(({ slug, shortName, item }, index) => (
              <div key={slug} className="grid gap-2">
                ...
                <Link
                  href={`/concerns/${slug}`}
                  className="px-1 text-[10px] font-bold text-pink underline underline-offset-4"
                >
                  Read the {shortName.toLowerCase()} guide
                </Link>
              </div>
            ))}
          </div>
      ...
      {!query && type === "all" && showDiscovery && (
        <section className="mt-8">
          <div className="flex items-end justify-between gap-4">
            <div>
              <p className="eyebrow">
                {combined ? "Or browse everything" : collectionEyebrow}
              </p>
              <h2 className="font-display text-4xl">
                {combined ? "All treatment collections" : collectionTitle}
              </h2>
            </div>
            {collection !== "all" && (
              <button
          ...
          {!combined && (
            <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
              {collectionCards.map(({ name, item }, index) => (
                <button
                  key={name}
                  type="button"
                ...
                >
                  ...
                  <span className="relative flex h-full items-end font-display text-xl leading-none">
                    {name}
                  </span>
                </button>
              ))}
          ...
          <details className="mt-4 rounded-2xl border border-black/5 bg-white p-4">
            <summary className="cursor-pointer text-xs font-bold text-pink">
      ...
      <div className={`grid gap-3 sm:gap-4 ${resultsColumns === "two" ? "grid-cols-2" : "sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4"}`}>
        {filtered.slice(0, visibleCount).map((item, index) => {
          const itemHref = combined
            ...
            : `/products-services/${branchSlug}/${item.handle}`;
          ...
          const inBasket = basketItems.some(
            (basketItem) =>
              basketItem.branchId === branchId &&
              basketItem.handle === item.handle,
          );
          return (
            <article
              key={item.handle}
              className="group flex h-full flex-col overflow-hidden rounded-2xl border border-black/5 bg-white shadow-soft"
            >
              <Link
                href={itemHref}
              ...
              <div className={`flex flex-1 flex-col ${resultsColumns === "two" ? "p-3 sm:p-5" : "p-5"}`}>
                ...
                <div className="mt-3 flex items-baseline gap-2">
                  <p className="text-sm font-bold text-pink">{price.current}</p>
                  {price.original && (
                    <p
                      className="text-xs text-black/40 line-through"
                      aria-label={`Original price ${price.original}`}
                    >
                      {price.original}
                    </p>
                  )}
                ...
                {combined && (
                  <Link
                    href={itemHref}
                    className="mt-auto inline-flex min-h-10 items-center justify-center rounded-full bg-pink px-4 text-xs font-bold text-white transition hover:bg-pink-dark"
                  >
                    View branch prices
                  </Link>
                )}
                {!combined && (
                  <>
                    {variants.length > 1 && (
                      <label className="mt-4 grid gap-1.5 text-[9px] font-bold uppercase tracking-[.12em] text-black/40">
                        Choose option
                        <select
                          value={variantIndex}
                          onChange={(event) =>
                            setSelectedVariants((current) => ({
                              ...current,
                              [item.handle]: Number(event.target.value),
                            }))
                          }
                          className="rounded-xl border border-black/10 bg-cream px-3 py-2.5 text-xs font-bold normal-case tracking-normal text-ink outline-none focus:border-pink"
                        >
                    ...
                    {variants.length ? (
                      <div className="mt-auto grid gap-2 pt-4">
                        <button
                          type="button"
                          onClick={() => addCatalogItem(item, variants)}
                          className="inline-flex min-h-10 items-center justify-center gap-2 rounded-full bg-pink px-4 text-xs font-bold text-white transition hover:bg-pink-dark"
                        >
                          {justAdded ? (
                            <>
                              <Check size={14} /> Added
                        ...
                        {inBasket ? (
                          <Link
                            href="/basket"
                            className="inline-flex min-h-10 items-center justify-center gap-2 rounded-full border border-pink/20 bg-pink-light/50 px-4 text-xs font-bold text-pink"
                          >
                            <ShoppingBag size={14} /> View basket
                          </Link>
                        ) : (
                          <Link
                            href={`/checkout/${branchSlug}/catalog/${encodeURIComponent(item.handle)}`}
                            className="text-center text-[10px] font-bold text-pink underline underline-offset-4"
                          >
                            Buy now
                          </Link>
                        )}
                      ...
                      <Link
                        href={`/contact?branchId=${branchId}&catalogItem=${encodeURIComponent(item.handle)}`}
                        className="mt-auto inline-flex pt-5 text-xs font-bold text-pink underline underline-offset-4"
                      >
                        Enquire
                      </Link>
                    )}
      ...
      {!filtered.length && (
        <div className="rounded-2xl bg-white p-10 text-center text-sm text-black/50">
