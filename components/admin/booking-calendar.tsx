...
import { ArrowLeft, ArrowRight, CalendarDays, CheckCircle2, Clock, Columns3, LoaderCircle, MapPin, Pencil, Plus, Trash2, UserRound, X, XCircle } from "lucide-react";
...
export function BookingCalendar({ initialBookings, branches, services, staff }: Props) {
  ...
  async function removeEditingBooking() {
    if (!editing || !window.confirm(`Delete booking for ${editing.customerName}? This cannot be undone.`)) return;
    setEditMessage({ type: "saving", message: "Deleting booking..." });
    const response = await fetch("/api/admin/bookings", { method: "DELETE", headers: { "content-type": "application/json" }, body: JSON.stringify({ id: editing.id }) });
