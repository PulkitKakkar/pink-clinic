...
import { parseLaserSettings, type LaserAreaSetting } from "@/lib/admin/laser-settings";
const cls =
...
function details(notes: string) {
  ...
  const laserAreas = parseLaserSettings(notes);
  return {
...
export function EditTreatmentRecord({ booking, treatmentNames }: { booking: Booking; treatmentNames: string[] }) {
  ...
  const [laserAreas, setLaserAreas] = useState<LaserAreaSetting[]>(record.laserAreas);
