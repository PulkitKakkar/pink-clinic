...
export function PaymentCheckoutMockup({ branch, service, price, payment }: Props) {
        ...
        <div className="overflow-hidden rounded-[2rem] border border-black/5 bg-white shadow-soft">
          ...
          <div className="p-6 text-center sm:p-10">
            ...
            <div className="mt-7 rounded-2xl bg-cream p-5 text-left">
              ...
              <div className="mt-3 flex justify-between gap-4 border-t border-black/5 pt-3"><span className="text-sm text-black/50">Total</span><span className="text-xl font-bold">£{price.toFixed(2)}</span></div>
            </div>
            ...
            <button type="button" disabled className="mt-6 flex min-h-12 w-full cursor-not-allowed items-center justify-center gap-2 rounded-full bg-black/15 px-6 text-sm font-bold text-white"><Lock size={15} /> Pay £{price.toFixed(2)} · Demo only</button>
  ...
  return (
    <section className="container-site grid gap-7 py-10 sm:py-16 lg:grid-cols-[1.05fr_.95fr] lg:gap-12 lg:py-20">
      <div>
        <Link href={`/treatments/${branch.slug}/${service.slug}`} className="inline-flex items-center gap-2 text-xs font-bold text-pink"><ArrowLeft size={15} /> Back to treatment</Link>
      ...
      <aside className="h-fit rounded-[2rem] bg-[#210013] p-6 text-white shadow-soft sm:p-8 lg:sticky lg:top-28">
        ...
        <p className="mt-2 text-xs text-white/50">{service.duration}</p>
        ...
        <div className="flex items-center justify-between py-6"><span className="text-sm text-white/55">Total</span><span className="text-3xl font-bold">£{price.toFixed(2)}</span></div>
