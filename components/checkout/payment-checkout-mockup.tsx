...
export function PaymentCheckoutMockup({ branch, service, price, payment, variants = [], returnHref, returnLabel = "Back to treatment" }: Props) {
  ...
  return (
    <section className="container-site grid gap-7 py-10 sm:py-16 lg:grid-cols-[1.05fr_.95fr] lg:gap-12 lg:py-20">
      <form id="treatment-customer-details" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); setCustomer({ firstName: String(form.get("firstName") || ""), lastName: String(form.get("lastName") || ""), email: String(form.get("email") || ""), phone: String(form.get("phone") || ""), address: String(form.get("address") || ""), postcode: String(form.get("postcode") || "") }); setShowProvider(true); }}>
