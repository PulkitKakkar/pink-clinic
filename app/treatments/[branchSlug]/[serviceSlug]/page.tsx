...
export function generateStaticParams() { return branches.flatMap((branch) => services.map((service) => ({ branchSlug: branch.slug, serviceSlug: service.slug }))); }
...
export default async function BranchServicePage({ params }: { params: Promise<{ branchSlug: string; serviceSlug: string }> }) {
  ...
  const price = formatTreatmentPrice(treatmentPrice);
