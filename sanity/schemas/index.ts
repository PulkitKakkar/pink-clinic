...
export const schemaTypes = [
  ...
  defineType({
    ...
    fields: [
      ...
      defineField({ name: "concerns", title: "Customer concerns", type: "array", group: "display", description: "Choose the customer problems this treatment can be browsed under.", of: [{ type: "string" }], options: { list: [
        { title: "Lines & wrinkles", value: "lines-wrinkles" }, { title: "Pigmentation", value: "pigmentation" }, { title: "Skin boosters", value: "skin-boosters" }, { title: "Acne & texture", value: "acne-texture" }, { title: "Unwanted hair", value: "unwanted-hair" }, { title: "Body contouring", value: "body-contouring" },
      ] } }),
      defineField({ name: "expectedResults", title: "Expected results", type: "array", group: "details", description: "Use careful, realistic wording. Do not guarantee outcomes.", of: [{ type: "string" }] }),
      defineField({ name: "treatmentAreas", title: "Treatment areas", type: "array", group: "details", of: [{ type: "string" }] }),
      defineField({ name: "duration", title: "Typical appointment duration", type: "string", group: "details", description: "For example, 30–45 minutes." }),
      defineField({ name: "downtime", title: "Typical downtime", type: "string", group: "details", description: "Explain that individual response may vary." }),
      defineField({ name: "sessions", title: "Typical treatment plan", type: "string", group: "details", description: "For example, a course may be recommended after consultation." }),
      defineField({ name: "faqs", title: "Frequently asked questions", type: "array", group: "details", of: [{ type: "object", fields: [defineField({ name: "question", title: "Question", type: "string", validation: rule => rule.required() }), defineField({ name: "answer", title: "Answer", type: "text", rows: 4, validation: rule => rule.required() })] }] }),
      defineField({ name: "beforeAfter", title: "Before and after results", type: "array", group: "details", description: "Only upload genuine client results with documented consent.", of: [{ type: "object", fields: [defineField({ name: "before", title: "Before", type: "image", options: { hotspot: true }, validation: rule => rule.required() }), defineField({ name: "after", title: "After", type: "image", options: { hotspot: true }, validation: rule => rule.required() }), defineField({ name: "alt", title: "Image description", type: "string" }), defineField({ name: "consentConfirmed", title: "Client consent confirmed", type: "boolean", validation: rule => rule.required().custom(value => value === true || "Confirm documented client consent before publishing.") })] }] }),
